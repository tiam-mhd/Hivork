...
import {
  ...
  assertStaffRoleMutationAllowed,
} from './staff-role-mutation-auth.js';

export type RemoveRoleFromStaffInput = {
  tenantId: string;
  actorId: string;
  staffId: string;
  roleId: string;
...
export type RemoveRoleFromStaffOutput = {
  removed: true;
};

export class RemoveRoleFromStaffUseCase
  implements UseCase<RemoveRoleFromStaffInput, RemoveRoleFromStaffOutput>
{
  constructor(
  ...
  async execute(input: RemoveRoleFromStaffInput): Promise<RemoveRoleFromStaffOutput> {
    ...
    const assignment = await this.staffRoles.findActiveAssignment(
      input.tenantId,
      staffRecord.id,
      role.id,
    );
    if (!assignment) {
      throw new ApplicationError(
        'ROLE_NOT_FOUND',
        'Role is not assigned to this staff member.',
        404,
      );
    }

    await assertNotLastOwnerRemoval(this.staffRoles, input.tenantId, role.code);

    await this.staffRoles.remove(input.tenantId, staffRecord.id, role.id);
