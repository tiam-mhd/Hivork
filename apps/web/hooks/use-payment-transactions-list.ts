...
import { listPaymentTransactions, type ListPaymentTransactionsParams } from '@/lib/api/payments';

export type PaymentTransactionsFilters = {
  status?: ListPaymentTransactionsParams['status'];
...
export function usePaymentTransactionsList(filters: PaymentTransactionsFilters) {
  const { resolve } = useApiError();
  const [items, setItems] = useState<PaymentTransactionListItemDto[]>([]);
