...
test.describe('branches page', () => {
  ...
  test('owner can view branches list', async ({ page, request }) => {
    await loginAsOwner(page, request);
    await page.goto('/admin/branches');

    await expect(page.getByRole('heading', { name: 'شعب' })).toBeVisible();
    await expect(page.getByRole('button', { name: '＋ شعبه جدید' })).toBeVisible();
  });

  test('default branch delete button is disabled', async ({ page, request }) => {
    await loginAsOwner(page, request);
    await page.goto('/admin/branches');

    const defaultBadge = page.getByText('شعبه پیش‌فرض').first();
    await expect(defaultBadge).toBeVisible({ timeout: 15_000 });

    const row = page.locator('tr').filter({ hasText: 'شعبه پیش‌فرض' }).first();
