...
test.describe('overdue report page', () => {
  test.beforeAll(async ({ request }) => {
    const health = await request.get(`${API_URL.replace('/api/v1', '')}/health`).catch(() => null);
    test.skip(!health?.ok(), 'API is not running');
  });

  test('shows overdue report with disabled export', async ({ page, request }) => {
    await loginAsOwner(page, request);
    await page.goto('/admin/reports/overdue');

    await expect(page.getByRole('heading', { name: 'گزارش معوقات' })).toBeVisible();
    await expect(page.getByLabel('شعبه')).toBeVisible();
    await expect(page.getByRole('button', { name: 'خروجی Excel' })).toBeDisabled();
  });

  test('sort filter updates URL', async ({ page, request }) => {
    await loginAsOwner(page, request);
    await page.goto('/admin/reports/overdue');

    await page.getByLabel('مرتب‌سازی').selectOption('overdueDays:desc');
    await expect(page).toHaveURL(/sort=overdueDays/);
  });
});
