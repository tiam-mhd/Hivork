...
test.describe('dashboard page', () => {
  ...
  test('loads KPI cards after login', async ({ page, request }) => {
    await loginAsOwner(page, request);

    await expect(page.getByRole('heading', { name: 'داشبورد' })).toBeVisible();
    await expect(page.getByText('معوقات')).toBeVisible();
    await expect(page.getByText('دریافتی این ماه')).toBeVisible();
    await expect(page.getByText('فروش‌های فعال')).toBeVisible();
    await expect(page.getByRole('heading', { name: 'قسط‌های سررسید امروز' })).toBeVisible();
  });
});
