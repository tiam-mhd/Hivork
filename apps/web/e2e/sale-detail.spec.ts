...
test.describe('sale detail page', () => {
  ...
  test('cancels active sale without paid installments', async ({ page, request }) => {
    await loginAsOwner(page, request);
    ...
    await page.getByRole('button', { name: 'لغو فروش' }).click();
    await page.getByLabel('دلیل لغو').fill('مشتری درخواست لغو فروش را داد');
    await page.getByRole('button', { name: 'لغو فروش', exact: true }).click();

    await expect(page.getByText('فروش با موفقیت لغو شد')).toBeVisible({ timeout: 10_000 });
    await expect(page.getByText('این فروش لغو شده است')).toBeVisible();
