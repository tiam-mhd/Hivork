...
test.describe('customer list page', () => {
  ...
  test('shows customers after login', async ({ page, request }) => {
    await loginAsOwner(page, request);
    await page.goto('/admin/customers');

    await expect(page.getByRole('heading', { name: 'مشتریان' })).toBeVisible();
    await expect(page.getByLabel('جستجو')).toBeVisible();
  });

  test('search filters results', async ({ page, request }) => {
    await loginAsOwner(page, request);
    await page.goto('/admin/customers');

    const search = page.getByLabel('جستجو');
    await search.fill('00000009999');
    await expect(page).toHaveURL(/search=00000009999/);
